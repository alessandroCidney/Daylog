<template>
  <div>
    <h2 class="mb-10">Data</h2>

    <h3 class="mb-5">Download data</h3>

    <p>You can download your data by using the following options:</p>

    <v-btn block class="mb-2 space--text" depressed>Download my account data</v-btn>
    <v-btn block class="mb-2 space--text" depressed>Download my posts data</v-btn>
    <v-btn block class="mb-2 space--text" depressed>Download all my data</v-btn>

    <h3 class="mb-5">Remove data</h3>

    <p>You can remove your data by using the following options:</p>

    <v-btn block class="mb-2 error--text" depressed>Delete my posts data</v-btn>
    <v-btn block class="mb-2 error--text" depressed>Delete all my data</v-btn>
  </div>
</template>